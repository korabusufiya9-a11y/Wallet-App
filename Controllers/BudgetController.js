...
exports.getAllBudgets = async (req, res) => {
    try {
        ...
        const budgetsWithAnalysis = await Promise.all(budgets.map(async (budget) => {
            ...
            const variance = calculateBudgetVariance(budget.limiteAmount, totalSpent);
...
exports.getBudgetById = async (req, res) => {
    try {
        ...
        if (!budget) return res.status(404).json({ success: false, message: "Budget is not found" });
...
exports.deleteBudget = async (req, res) => {
    try {
        ...
        res.status(200).json({ success: true, message: "Budget delete " });
    } catch (error) {
        res.status(500).json({ success: false, message: error.message });
    }
};